...
test_that("counts are coerced to integer", {
  ...
  ctl <- ripr_control(n_seeds = 10, n_restarts = 2, fc_max_iter = 5)
  expect_type(ctl$n_seeds, "integer")
  expect_type(ctl$n_restarts, "integer")
  expect_type(ctl$fc_max_iter, "integer")
  expect_type(ctl$fc_tol, "double")
})
...
test_that("counts must be whole numbers", {
  ...
  expect_error(ripr_control(fc_max_iter = 10.5), "whole number")
})
...
test_that("counts respect their lower bounds", {
  ...
  expect_error(ripr_control(fc_max_iter = 0), "whole number")
})

test_that("the tolerance must be a single non-negative number", {
  expect_silent(ripr_control(fc_tol = 0))
  expect_error(ripr_control(fc_tol = -1), "larger than or equal to 0")
  expect_error(ripr_control(fc_tol = c(1e-8, 1e-9)), "must be a number")
})
...
test_that("non-numeric input is rejected", {
  ...
  expect_error(ripr_control(fc_tol = "small"), "must be a number")
})
